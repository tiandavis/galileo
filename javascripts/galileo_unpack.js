...
exports.OAuth = (function (global) {
    ...
    function URI(url) {
        var args = arguments, args_callee = args.callee,
			...
			//Galileo
			//https://search.twitter.com/search.json?rpp=3&q=#Rails
			//Was treating #Rails like a hash and removing it from query. Leaving query this:
			//https://search.twitter.com/search.json?rpp=3&q=#Rails
            //parser = /^([^:\/?#]+?:\/\/)*([^\/:?#]*)?(:[^\/?#]*)*([^?#]*)(\?[^#]*)?(#(.*))*/
			parser = /^([^:\/?]+?:\/\/)*([^\/:?]*)?(:[^\/?]*)*([^?]*)(\?[^]*)?((.*))*/
            uri = this;
        ...
        if (url !== null) {
            parsed_uri = url.match(parser); //console.log(parsed_uri);
    ...
    OAuth.prototype = {
        ...
        init: function (options) {
            ...
            this.request = function (options) {
                ...
                url = URI(options.url); //console.log(url);
                data = options.data || {};
...
(function() {
  ...
  // HTML attribute for robot nofollow behavior (default)
  var HTML_ATTR_BLANK_TARGET = " target=\"_blank\"";
  ...
  twttr.txt.autoLinkUsernamesOrLists = function(text, options) {
    ...
    if (!options.suppressNoFollow) {
      var extraHtml = HTML_ATTR_NO_FOLLOW + HTML_ATTR_BLANK_TARGET;
    }
